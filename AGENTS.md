...
1. `nvm use` (installs Node 24.2.0 if missing) or install Node 24.x manually.
2. `pnpm run d` to install dependencies, rebuild native modules, and launch Electron + Vite.3. If `pnpm run d` fails mid-stream, rerun `pnpm install`, then `pnpm run dev` (main + renderer).
