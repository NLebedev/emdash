...
export class RemoteGitService {
  ...
  async getStatusDetailed(connectionId: string, worktreePath: string): Promise<GitChange[]> {
    ...
    for (const line of statusLines) {
      ...
      const hasUnstaged = statusCode[1] !== ' ';
      ...
      const unstagedAdditions = unstaged?.add ?? 0;
        ...
        for (let i = 0; i < untrackedPaths.length && i < counts.length; i++) {
          ...
          if (count >= 0) {
            ...
            if (change) {
              ...
              change.unstagedAdditions = count;
            }
          }
        }
      }
  ...
  async getFileDiff(
    ...
    let diffCmd = 'git diff --no-color --unified=2000';
    let originalRef = 'HEAD';
    let modifiedRef = 'disk';

    if (scope === 'staged') {
      diffCmd += ` --cached -- ${quoteShellArg(filePath)}`;
      originalRef = 'HEAD';
      modifiedRef = 'index';
    } else if (scope === 'unstaged') {
      diffCmd += ` -- ${quoteShellArg(filePath)}`;
      originalRef = 'index';
      modifiedRef = 'disk';
    } else {
      diffCmd += ` HEAD -- ${quoteShellArg(filePath)}`;
      originalRef = 'HEAD';
      modifiedRef = 'disk';
    }

    // Step 1: Run git diff
    const diffResult = await this.sshService.executeCommand(connectionId, diffCmd, cwd);
    ...
    // Step 3: Fetch content (non-binary only)
    const getShowCmd = (ref: string) =>
      `s=$(git cat-file -s ${ref}:${quoteShellArg(filePath)} 2>/dev/null); ` +
      `if [ "$s" -le ${MAX_DIFF_CONTENT_BYTES} ] 2>/dev/null; then git show ${ref}:${quoteShellArg(filePath)}; ` +
    ...
    const originalCmd = originalRef === 'index' ? getShowCmd('') : getShowCmd('HEAD');
    const modifiedCmd = modifiedRef === 'index' ? getShowCmd('') : getCatCmd();

    const [showResult, catResult] = await Promise.all([
      this.sshService.executeCommand(connectionId, originalCmd, cwd),
      this.sshService.executeCommand(connectionId, modifiedCmd, cwd),
    ]);
